...
[y_new,xf1_new] = myNeuralNetworkFunction_io(all_voltages.', xi1.');

%% Save variables to file
threshold = 0.5;
y_thresh = y_new;
y_thresh(y_new < threshold) = 0;
y_thresh(y_new > threshold) = 0.95;

plot(1:length(y_thresh),all_arcs,1:length(y_thresh),y_thresh),
title('Predicted against actual short circuit - Testing on first3files'),
grid on
